...
class LinRegTrainingDataHandler(val fileName: String) extends TrainingDataHandler {
  ...
  def finish() = {
    ...
    if (trainingData.nonEmpty) {
      ...
      SpotlightLog.info(this.getClass(), "Weights: %s", weightsTsvString)
...
/**
 * Creates a vowpal compatible training set file.
 * Weights can be generated by using vowpal wabbit as follows: vw fileName --invert_hash fileName.model
 */
class DumpVowpalTrainingDataHandler(val fileName: String) extends TrainingDataHandler {
  ...
  def addCase(target: Double, features: List[(String, Double)], occ: DBpediaResourceOccurrence) = {
    ...
    writer.println("%f %s[%d]->%s|%s".format(target, occ.surfaceForm.name, occ.textOffset, occ.resource.uri, featuresString))
  }
